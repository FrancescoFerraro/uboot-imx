...
#define CFG_EXTRA_ENV_SETTINGS	\
	...
	"mmcargs=setenv bootargs ${cpuidle} ${mcore_args} console=${console} root=/dev/mmcblk${mmcblk}p${mmcpart} rootwait rw clk_ignore_unused pd_ignore_unused\0 " \
	"script=boot.scr\0" \
	"bootenv=uEnv.txt\0" \
	...
	"use_m7=no\0" \
	"loadm7bin=load mmc ${mmcdev}:${mmcpart} ${loadaddr} ${bootdir}/${m7_bin} && cp.b ${loadaddr} ${m7_addr} ${filesize};\0" \
	"runm7bin=echo Booting M7 from TCM; stopaux 1; bootaux ${m7_addr_auxview} 1;\0" \
