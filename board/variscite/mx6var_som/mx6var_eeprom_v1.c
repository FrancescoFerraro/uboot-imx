...
static int var_eeprom_write(struct udevice *dev, uchar *ptr, u32 size, u32 eeprom_i2c_addr, u32 offset)
{
	int ret = 0;
	u32 write_len;
	u32 size_left = size;
	...
	while ((ret == 0) && (size_left > 0)) {
		...
		ret = dm_i2c_write(dev, addr, ptr, write_len);
...
static int do_var_eeprom_params(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (var_eeprom_write(dev, (uchar *) var_eeprom_v1_cfg.header.part_number,
	...
	if (var_eeprom_write(dev, (uchar *) var_eeprom_v1_cfg.header.assembly,
	...
	if (var_eeprom_write(dev, (uchar *) var_eeprom_v1_cfg.header.date,
